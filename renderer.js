...
function render(){
 ...
 <span class="${t.priority}">${priorityLabel(t.priority)}</span><span>${t.date?formatDate(t.date):""}${t.time?" · "+t.time:""}</span><span>${esc(projectName(t.projectId))}</span></div></div>
...
function notifyTask(task){
 ...
 if(window.taskforge) window.taskforge.showNotification({title:"TaskForge",body:`⏰ ${task.title}`});
}
function scheduleNotifications(){
 const now=Date.now();
 for(const [id,timer] of scheduled){clearTimeout(timer); scheduled.delete(id);}
 for(const task of data.tasks){
   if(task.completed||task.deleted||!task.date||!task.time) continue;
   ...
   const timer=setTimeout(()=>{notifyTask(task);scheduled.delete(task.id);},delay);
...
$("taskForm").onsubmit=e=>{e.preventDefault();let title=$("taskTitle").value.trim();if(!title)return;
 ...
 data.tasks.push({id:uid(),title,priority:$("taskPriority").value,date:$("taskDate").value,time:$("taskTime").value,projectId:pid||"inbox",completed:false,deleted:false});
